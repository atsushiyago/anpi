...
- **`/dashboard`と`/api/*`にBasic認証を追加(2026-08-02)**: `src/proxy.ts`
  (username固定`demo`、パスワードは`DEMO_PASSWORD`環境変数)。`DEMO_PASSWORD`未設定時は
  常に拒否(fail closed)。ローカルで無認証/誤パスワード/正しいパスワード/未設定時、
  すべて期待通りの401/200を確認済み。Vercel(Production/Preview)にも`DEMO_PASSWORD`を
  設定済み。**ただしコードはまだコミット・デプロイしていない**ので、本番の`/dashboard`は
  まだBasic認証がかかっていない(次回デプロイ時に有効化される)。
