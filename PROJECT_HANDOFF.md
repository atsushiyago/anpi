...
14. **Next.js 16では`middleware.ts`が非推奨**、`proxy.ts`にリネームされている
    (エクスポートする関数名も`middleware`→`proxy`)。挙動・`config.matcher`の書き方は同じ。
    知らずに`middleware.ts`を作るとdevサーバー起動時に非推奨警告が出る
    (`node_modules/next/dist/docs/.../proxy.md`参照)。

---

## 現在の状態(2026-08-02時点)

- **`/dashboard`と`/api/*`にBasic認証を追加、本番デプロイ済み(2026-08-02)**: `src/proxy.ts`
  (username固定`demo`、パスワードは`DEMO_PASSWORD`環境変数)。`DEMO_PASSWORD`未設定時は
  常に拒否(fail closed)。ローカル・本番とも無認証401/正しいパスワード200を確認済み。
  Vercel(Production/Preview)にも`DEMO_PASSWORD`設定済み。
  なお、最初に生成したパスワードが会話ログに平文で貼られたため、1回ローテーション済み
  (現在Vercelに設定されているのはローテーション後の値)。
...
- **本番デプロイ済み**: https://call-e-anpi.vercel.app (Vercel, GitHubの`main`ブランチと連携済み)。
  ...
  i18n対応もデプロイ済み。既存recipientの`locale`自動補完("ja")も本番で確認済み。
  英語通話自体(日本以外の番号)の本番確認はまだ。
...
1. (完了済み)**公開デモの扱い**
   - データ保存はUpstash Redis(REST API、リージョン: AWS us-east-1)。Vercel本番/Preview両方に
     環境変数設定済み。
   - **実データのリセット完了(2026-08-02)**: 本番Redisを確認したところ、以前の削除ボタンの
     動作確認で既に空(recipients/callsとも0件)になっていた。ローカルに残っていた旧
     `data/wellness-data.json`(実名・電話番号・メールアドレス入り)も削除済み。
     移行用の`scripts/migrate-json-to-redis.mts`と`npm run migrate:to-redis`も、
     移行元ファイルが無くなり不要になったため削除(README・package.jsonからも参照を除去)。
   - `/dashboard`・`/api/*`にBasic認証も導入済み(13番参照)なので、審査員に自分の電話番号を
     対象者として登録してもらう際は、事前にBasic認証のusername/passwordを伝える必要がある。
